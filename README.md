# Hub de Juegos para TikTok LIVE

Juegos interactivos para streamers de TikTok LIVE, pensados para usarse como Browser Source en OBS, Streamlabs o TikTok LIVE Studio.

Repositorio público: https://github.com/mylnzon58/tiktok-live-games
...
- `arena.html`, `arena.js` y `arena.css` (raíz) son el Plinko de Avatares.
- Cada flujo debe tratarse como un dominio separado que solo comparte infraestructura.
...
- `bomba/bomba-manager.js` y `carrera/carrera-manager.js`: estado autoritativo de los juegos nuevos.
...
El servidor se configura con variables de entorno, que también pueden guardarse en un archivo `.env`:
...
Si se necesita sesión autenticada, se usan variables de entorno en vez de cookies hardcodeadas:
...
## Usar los juegos en OBS / TikTok LIVE Studio

1. Inicia el LIVE en TikTok con la cuenta configurada.
2. Abre `http://localhost:3000/` y elige el juego.
3. En OBS, Streamlabs o TikTok LIVE Studio, agrega una fuente **Browser Source** con la URL del juego.
...
Gift battle por equipos, basada en los formatos más rentables del mercado:
...
- Los regalos de fuego/rayo (Fireworks, Dragon Flame, Galaxy, Planet) o de 1000+ diamantes causan **CAOS**: empujan a tu equipo y hacen retroceder al rival (el regalo del villano, el más rentable).
- El equipo perdedor recibe multiplicador ×1.3 (catch-up): mantiene la pelea pareja, lo que incentiva más regalos.
- Combos: 3/5/10 regalos seguidos del mismo equipo → ×1.1/×1.25/×1.5.
- Los likes cargan la barra de poder del equipo; al llegar a 200, disparan un impulso +80.
- **EMPUJE FINAL**: los últimos 30 segundos valen el doble (×2) con overlay dorado.
- Victoria instantánea al llegar a 8000 puntos, o por score al terminar los **5 minutos**.
- El MVP del equipo ganador (mayor donante) se persiste en `titan_hof.json`.
- Los likes se acumulan por lote (flush cada 100 ms) para soportar audiencias grandes.

### Tap Tap Arena (`/arenagame`)

Reglas fijas que no deben revertirse:

- Combate PvP: cada avatar tiene HP, se ataca con regalos, y los likes (tap tap) curan y disparan.
- Ganador de la ronda: jugador individual con mayor `standingScore`.
- Número uno del arena (REY): jugador con más RONDAS ganadas.
- Si empatan en RONDAS, desempata por score actual y luego por `bestScore`.
- Los regalos son mucho más fuertes que los taps.
...
- Los jugadores inactivos no deben contaminar el ranking vivo ni las locuciones.

### Plinko de Avatares (`/arena`)

- Cada regalo hace caer bolas-avatar por la pirámide: Rosa → 1 bola, 99+ diamantes → 5, 1000+ → 10, Galaxia → 20, León → bola gigante.
...
### Combate PvP y eventos

`server.js` implementa la arena de combate completa (posiciones, HP autoritativo, estados, sierras, strikes, death match, frenzy, golden minute, powerups). El cliente Plinko solo renderiza una parte; el cliente del Tap Tap Arena (`/arenagame`) consume el flujo PvP completo.
...
| Frenzy | 3000 diamantes acumulados en la ronda → 30 s de frenesí |
| Minuto de Oro | Aleatorio entre 30 s y la mitad de la ronda, 60 s de puntuación x2 |
...
3. Variables de entorno:
   - `PORT`: el que asigne Railway (automático).
...
Los contratos completos por juego están en [docs/SOCKET-CONTRACTS.md](docs/SOCKET-CONTRACTS.md) y la descripción de la arquitectura en [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
| `arena:likesBatch` | Lote de likes/taps agrupados (flush 16 ms) |
...
| `bigGift` | Regalo de 1000+ diamantes |
...
## Actualizar el catálogo de regalos
...
- El sistema usa el `diamondCount` real del evento.
