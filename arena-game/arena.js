...
socket.on("status", () => {
    // Solo feedback en terminal del servidor (consola web removida por limpieza).
});
...
socket.on("arena:globalKing", (king) => {
    ...
    currentTopArenaLeader = {
        id: king.id,
        name: king.name,
        avatar: king.avatar || currentTopArenaLeader?.avatar || "",
        victories: king.victories || 0
    };
});
...
function handleArenaLikeBatchItem(data) {
    ...
    const comboLikes = data.comboLikes || likeCount;
    ...
    spawnFloatingText(`TAP +${Math.max(1, Math.floor(likeCount || 1))}`, p.x, p.y - 10, "#86efac");
    if (data.scoreGain > 0) {
    ...
    if (comboLikes >= 16 && comboLikes % 8 === 0) {
        spawnFloatingText(`RUSH x${comboLikes}`, p.x, p.y - 54, "#7dd3fc");
        triggerOverlayFlash("120, 255, 210", 0.04);
        screenShake = Math.max(screenShake, Math.min(8, 2 + (comboLikes / 30)));
    }
    ...
    if (strikes > 0 && strikes % 50 === 0) {
        ...
        screenShake = Math.max(screenShake, 4);
    } else if (likeCount >= 3 || strikes % 6 === 0) {
