...
function createArenaManager() {
    ...
    function getArenaSpawn() {
        const radius = arenaConfig.circleRadius || 350;
        const r = radius * 0.7;
        const a = Math.random() * Math.PI * 2;
        const d = Math.sqrt(Math.random()) * r;
        return {
            x: Math.round(400 + Math.cos(a) * d),
            y: Math.round(675 + Math.sin(a) * d) // Centro ajustado para 1350 de alto (1350/2 = 675)
        };
    }
    ...
    function respawnPlayer(player, mode = "basic", now = Date.now()) {
        ...
        player.x = spawn.x;
        player.y = spawn.y;
        player.hp = arenaConfig.maxHp; // Resurgen con la vida completa
        player.invulnerableUntil = now + 3500; // Inmunidad gigante para ayudarles a escapar/crecer al inicio
    ...
    function getCurrentRanking(limit = 10) {
        return Object.values(arenaPlayers)
            ...
            // Permitir IDLE para que se pueda anunciar ganador aunque el líder esté quieto al final
            .filter((player) => player.state !== PLAYER_STATE.REMOVED)
            .sort((a, b) =>
    ...
    function applyDamageConsequences(attacker, target, damage, now = Date.now(), isPaidGift = false) {
        let scoreLoss = 0;
        let ko = false;
        let finalDamage = damage;

        if (now >= (target.invulnerableUntil || 0)) {
            // ESCUDO DE PRINCIPIANTE / UNDERDOG:
            // Si el objetivo tiene poco puntaje (< 1000) y el atacante es MUCHÍSIMO más grande, el objetivo recibe armadura.
            // El atacante GRATUITO sigue viéndose épico, pero el pequeñito no es borrado instantáneamente.
            // EXCEPCIÓN: ¡Si el atacante pagó un regalo real (isPaidGift), su mérito destruye el escudo!
            if (!isPaidGift && target.score < 1000 && attacker.score > (target.score * 3)) {
                // Ratio de diferencia de tamaño (maximo capado a 15)
                const sizeRatio = Math.min(15, attacker.score / Math.max(1, target.score));
                // A mayor diferencia, menor es la proporción de daño que pasa al jugador pequeño.
                finalDamage = Math.max(8, Math.floor(damage / Math.sqrt(sizeRatio)));
            }
            ...
            if (target.hp <= 0) {
                ...
                const retainedScore = Math.max(0, Math.floor(scoreBeforeKoReset * arenaConfig.koScoreRetainRatio));
                const koLossApplied = Math.max(
                    Math.min(arenaConfig.koScoreLossBonus, scoreBeforeKoReset),
    ...
    function applyGiftCombat(attackerId, targetId, resolution, isSuddenDeath = false) {
        ...
        const scoreGain = Math.max(1, Math.round(resolution.totalDiamonds * GIFT_VALUE_BOOST * finalScoreScale * comboMultiplier * scoreMultiplier));
        ...
        const outcome = applyDamageConsequences(attacker, target, damage, now, true);
    ...
    function applySawAuraHits(isSuddenDeath = false, now = Date.now()) {
        ...
        for (const attacker of attackers) {
            ...
            // Dominancia: Si el atacante es más grande, le roba puntos al objetivo
            const targetScore = closestTarget.score || 0;
            if (attacker.score > targetScore * 1.2) {
                const drain = Math.min(targetScore, Math.ceil(damage * 0.5));
                closestTarget.score -= drain;
                attacker.score += drain;
            }
