...
function createArenaManager() {
    ...
    function resetRound() {
        ...
        for (const player of Object.values(arenaPlayers)) {
            ...
            player.hp = arenaConfig.maxHp;
            player.comboCount = 0;
            player.state = idleFor >= arenaConfig.idleVisualAfterMs ? PLAYER_STATE.IDLE : PLAYER_STATE.ACTIVE;
            player.idleSince = player.state === PLAYER_STATE.IDLE ? (player.idleSince || now) : 0;
            player.eliminatedUntil = 0;
