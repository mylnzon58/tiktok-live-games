...
function createArenaManager() {
    ...
    function sanitizePlayerForClient(player) {
        ...
        const standingScore = Math.max(
            0,
            Math.round((player.score || 0) + ((player.hp || 0) * arenaConfig.roundHpWeight) - (deaths * arenaConfig.roundDeathPenalty))
        );
        return {
            id: player.id,
            name: player.name,
    ...
    function applyLikeStrike(playerId, comboLikes) {
    ...
    function cleanup(now = Date.now()) {
        ...
        for (const [id, player] of Object.entries(arenaPlayers)) {
            ...
            const hasProtectedChampionStatus = Boolean(arenaHallOfFame[id]) && idleFor < arenaConfig.idleVisualAfterMs;
            const removeAfter = hasProtectedChampionStatus
                ? arenaConfig.removeChampionAfterMs
                : arenaConfig.removeInactiveAfterMs;
