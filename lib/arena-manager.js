...
function createArenaManager() {
    ...
    const HOF_SAVE_INTERVAL_MS = 5000; // Como máximo, un guardado cada 5 segundos
        ...
        hofSaveTimer = setTimeout(() => {
            ...
            try {
                fs.writeFile(HOF_FILE, JSON.stringify(arenaHallOfFame), (err) => {
                    if (err) console.error("Error al guardar el salón de la fama:", err.message);
                });
            } catch (error) {
                console.error("Error al guardar el salón de la fama:", error.message);
            }
        }, HOF_SAVE_INTERVAL_MS);
    }

    // Guardado inmediato para un apagado ordenado
    function saveHOFSync() {
        try {
            fs.writeFileSync(HOF_FILE, JSON.stringify(arenaHallOfFame));
        } catch (error) {
            console.error("Error al guardar el salón de la fama (síncrono):", error.message);
        }
    }
    ...
    function loadHOF() {
        ...
        try {
            ...
            console.error("Error al cargar el salón de la fama:", error.message);
            arenaHallOfFame = {};
        }
    }
    ...
    function clampPosition(x, y, radius = 22) {
    ...
    function estimateRadiusFromScore(score = 0) {
        ...
        const scoreScale = Math.min(
            (Math.sqrt(safeScore) * 0.85) + (Math.log2(safeScore + 1) * 2),
            75
        );
        return 30 + scoreScale;
    }
    ...
    function getPlayers() {
        // Se envían todos los jugadores no removidos para mantener la visibilidad completa
        const result = {};
    ...
    function getCurrentRanking(limit = 10) {
        return Object.values(arenaPlayers)
            ...
            // Prioriza la puntuación de la ronda (meritocracia en tiempo real)
            .sort((a, b) =>
    ...
    function pickTarget(attackerId) {
        ...
        // Preferencia de objetivo (neuromarketing)
        ...
        for (const player of Object.values(arenaPlayers)) {
            ...
            // Emparejamiento inteligente (restricción suave anti-bullying)
            ...
            // Penalización artificial en la distancia para no concentrar los ataques en los nuevos
    ...
    function applyDamageConsequences(attacker, target, damage, now = Date.now(), isPaidGift = false, canKill = true) {
        ...
        if (now >= (target.invulnerableUntil || 0)) {
            ...
            if (isUnderdog) {
                ...
                finalDamage = Math.min(finalDamage, arenaConfig.underdogMaxDamageCap || 250); // Tope máximo absoluto
            }
            ...
            // Para ataques pagos con bullying extremo
            ...
            if (isUnderdog) {
                // Protección extrema: los novatos no pierden puntos por daño pasivo ni ataques. 
            ...
            if (target.hp <= 0) {
                if (!canKill) {
                    ...
                    // Si es underdog, no se le aplica la pérdida fija adicional
    ...
    function applyGiftCombat(attackerId, targetId, resolution, isSuddenDeath = false) {
        ...
        // Golpes críticos (efecto near-miss, neuromarketing)
        ...
        // Mecanismo de David contra Goliat (neuromarketing)
        ...
        // Cualquier donador obtiene sierra VIP asegurada 30 segundos como mínimo; regalos grandes, 60 s.
        ...
        // El atacante conserva la sensación de golpe fuerte aunque el daño real esté limitado
        // Si el golpe fue a un novato (daño limitado), se registra el daño total en el ranking para premiar la inversión
        if (damage > outcome.damage && !isTargetUnderdog(target, now)) {
             // El atacante conserva una parte del daño bruto para el ranking
    ...
    function applyLikeSupport(playerId, likeCount) {
        ...
        // Bonificación de catch-up: 5 puntos si es nuevo o respawneó y tiene poca puntuación
        ...
        if (typeof updateHOF === 'function') updateHOF(player);
    ...
    function applyBucketBonus(playerId, mult) {
        const player = arenaPlayers[playerId];
        if (!player) return null;
        
        // Multiplicador real de puntos
        const scoreGain = mult * 15; // 15 pts base por bola
        player.score += scoreGain;
        player.bestScore = Math.max(player.bestScore || 0, player.score);
        return { scoreGain, totalScore: player.score };
    }

    function applyBombPenalty(playerId, lossAmount = 50) {
        const player = arenaPlayers[playerId];
        if (!player) return null;
        
        player.score = Math.max(0, player.score - lossAmount);
        return { scoreLoss: lossAmount, totalScore: player.score };
    }
    ...
    function applySawAuraHits(isSuddenDeath = false, now = Date.now()) {
        ...
        for (const attacker of attackers) {
            ...
            // El daño pasivo de la sierra no puede matar al objetivo
            const outcome = applyDamageConsequences(attacker, closestTarget, damage, now, false, false);
            
            // Robo de puntos (modo vampiro): el daño causado alimenta al atacante
            ...
            // Detectar choque de sierras (punta contra punta)
    ...
    function syncPosition(id, patch) {
        ...
        // Clamping forzado por el servidor para que no salgan de la arena
