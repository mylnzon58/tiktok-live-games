...
function createArenaManager() {
    ...
    function applySawAuraHits(isSuddenDeath = false, now = Date.now()) {
        ...
        for (const attacker of attackers) {
            ...
            const isTargetUnderdogLocal = targetScore < (arenaConfig.resilienceThresholdScore || 1500) || now < (closestTarget.resilienceUntil || 0);

            // ANTI-BULLYING EXTREMO: No robar puntos a los Underdogs (literalmente se desangran a 0 si están cerca)
            if (attacker.score > targetScore * 1.2 && !isTargetUnderdogLocal) {
