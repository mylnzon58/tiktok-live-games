...
function createArenaManager() {
    ...
    function applyBombPenalty(playerId) {
        const player = arenaPlayers[playerId];
        if (!player) return null;
        
        // Pierde el 15% de sus puntos actuales, con un mínimo de 10 pts
        const loss = Math.max(10, Math.floor(player.score * 0.15));
        player.score = Math.max(0, player.score - loss);
        return { scoreLoss: loss, totalScore: player.score };
    }
