...
function createArenaManager() {
    ...
    function createArenaPlayer(userData, previousRecord = null) {
        ...
        return {
            ...
            resilienceUntil: now + 30000 // 30 segundos de gracia al entrar por primera vez
        };
    }
    ...
    function respawnPlayer(player, mode = "basic", now = Date.now()) {
        ...
        player.y = spawn.y;
        player.hp = arenaConfig.maxHp; 
        player.invulnerableUntil = now + 5000; // 5 segundos de invulnerabilidad total absoluta
        player.resilienceUntil = now + 15000; // 15 segundos de resistencia extra (daño reducido)
    ...
    function applyDamageConsequences(attacker, target, damage, now = Date.now(), isPaidGift = false) {
        ...
        if (now >= (target.invulnerableUntil || 0)) {
            ...
            // Si el jugador acaba de revivir (resilienceUntil) o es muy pequeño (< 500 puntos), 
            // recibe un 60% menos de daño para que no lo borren de un golpe.
            const isUnderdog = target.score < 500 || now < (target.resilienceUntil || 0);
            if (isUnderdog) {
                finalDamage = Math.floor(finalDamage * 0.4); // Solo pasa el 40% del daño
            }
