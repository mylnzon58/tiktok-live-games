...
function readChromeSafeStorageKey() {
    for (const args of KEYCHAIN_PROBES) {
        try {
            ...
            // Continue through the known Keychain labels.
        }
    }
    return null;
}
...
function decryptChromeCookie(encryptedHex, key) {
    ...
    try {
        ...
        // Chrome cookie DB v24+ prefixes the plaintext with SHA256(host_key).
...
function collectTikTokCookies() {
    ...
    if (!profiles.length) {
        return { cookies: {}, errors: ["No se encontraron perfiles de Chrome con base de cookies."] };
    }

    const key = readChromeSafeStorageKey();
    if (!key) {
        return { cookies: {}, errors: ["No se pudo leer la clave 'Chrome Safe Storage' del Keychain."] };
    }
    ...
    return { cookies: freshestByName, errors };
}
...
function syncTikTokEnvFromChrome(options = {}) {
    ...
    if (process.platform !== "darwin") {
        return { updatedKeys: [], missingKeys: ["sessionid", "tt_target_idc"], errors: ["La auto-sincronizacion con Chrome solo esta implementada para macOS."] };
    }
    ...
    if (shouldLogStatus && missingKeys.length) {
        logger.log(`[tiktok-auth] No se pudo actualizar automaticamente: ${missingKeys.join(", ")}`);
    }
    if (shouldLogStatus) {
