...
const MAX_STREAK_AGE_MS = 30_000;  // Streaks viejos de 30s se purgan
const MAX_LIKE_ENTRIES = 2000;     // Máximo 2000 usuarios en likeState

setInterval(() => {
    const now = Date.now();

    // Purgar streaks viejos (si un streak no terminó en 30s, está stale)
...
function normalizeGiftEvent(raw = {}, giftCatalog) {
    ...
    if (fields.giftType === 1) {
        ...
        if (fields.repeatEnd) {
            ...
            // Guardar con timestamp para poder purgar streaks stale
